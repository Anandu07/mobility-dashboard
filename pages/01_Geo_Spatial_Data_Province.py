...
with col5:
    agg_age = province_data.groupby(['age']).agg({'trips': 'median'}).reset_index()
    fig_age = px.bar(agg_age, x='age',y='trips', title=f'Age Bin Distribution of Mobility in {selected_province}',
                     color='age', color_discrete_map=color_map)
    st.plotly_chart(fig_age)

with col6:
    agg_gender = province_data.groupby(['gender']).agg({'trips': 'median'}).reset_index()
    fig_gender = px.bar(agg_gender, x='gender',y='trips',title=f'Gender Distribution of Mobility in {selected_province}',
...
with col9:
    revenue_categories = ['<10', '10-15', '>15']
    agg_data_revenue_age = province_data.groupby(['age', 'revenue']).agg({'trips': 'median'}).reset_index()
    fig_agg_gender_trip_no = px.bar(agg_data_revenue_age, x='revenue', y='trips', color='age', barmode='group', color_discrete_map=color_map,category_orders={'revenue': revenue_categories},
